...
def analyze_submissions():
    ...
    return {
        ...
        "submissions": sorted(submissions, key=lambda x: x["modified"], reverse=True)[:10],  # Top 10
        "total_size_mb": round(total_size / (1024 * 1024), 2)
    }
...
def generate_report():
    ...
    if models_info['models']:
        ...
        for model in models_info['models'][:5]:  # Top 5
            print(f"  • {model['name']} - {model['size_mb']} MB - {model['modified'].strftime('%d/%m %H:%M')}")
    ...
    if datasets_info['datasets']:
        ...
        for dataset in datasets_info['datasets']:
            cols_info = f" - {dataset['columns']} colonnes" if dataset.get('columns') != '?' else ""
    ...
    if submissions_info['submissions']:
        ...
        for sub in submissions_info['submissions'][:3]:  # Top 3
            ...
            print(f"  • {sub['name']}{rows_info} - {sub['modified'].strftime('%d/%m %H:%M')}")
    ...
    total_size = models_info['total_size_mb'] + datasets_info['total_size_mb'] + submissions_info['total_size_mb']
    total_files = models_info['count'] + datasets_info['count'] + submissions_info['count']
    
    print("\n💫 RÉSUMÉ GLOBAL")
    print("-" * 30)
    print(f"📁 Total de fichiers générés: {total_files}")
    print(f"💾 Espace disque utilisé: {total_size:.2f} MB")
    print(f"🚀 Projet prêt: {'✅ Oui' if models_info['count'] > 0 else '❌ Non (aucun modèle)'}")
