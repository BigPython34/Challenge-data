...
def create_entity(params):
    ...
    es = es.add_dataframe(dataframe_name="clinical", dataframe=clinical, index="ID")

    es = es.add_dataframe(
        dataframe_name="molecular", dataframe=molecular, index="index"
    )
...
def main_preprocess(data, params):
    ...
    if "CYTOGENETICS" in clinical_process:
        data["clinical"] = parse_cytogenetics_column(
            data["clinical"].reset_index(drop=True), column_name="CYTOGENETICS"
        )

    elif "CYTOGENETICSv2" in clinical_process:
        data["clinical"] = parse_cytogenetics_column_v2(
            data["clinical"].reset_index(drop=True), column_name="CYTOGENETICS"
        )

    elif "CYTOGENETICSv3" in clinical_process:
        data["clinical"] = parse_cytogenetics_column_v3(
            data["clinical"].reset_index(drop=True), column_name="CYTOGENETICS"
        )

    elif "CYTOGENETICSv4" in clinical_process:
        data["clinical"] = parse_cytogenetics_column_v4(
            data["clinical"].reset_index(drop=True), column_name="CYTOGENETICS"
        )
    ...
    if "EFFECT" in molecular_process:
        effect = create_one_hot(
            data["molecular"],
            id_col="ID",
            ref_col="EFFECT",
            min_count=20,
            rare_label="effect_other",
        )
    ...
    for prcs in molecular_process:
        ...
        if "-" in prcs:
            col_to_soust, col_soust = prcs.split("-")
            soust = add_soustrac_column(
                data["molecular"], col_soust, col_to_soust, molecular=True
            )
            X = soust.merge(X, left_on="ID", right_index=True, how="right").set_index(
                "ID"
            )
...
def process_missing_values(
    X_train, X_test, X_eval, columns, strategy="median", sex=False
):
...
def create_one_hot(
    df, id_col="ID", ref_col="GENE", min_count=10, rare_label="gene_other"
):
    """
...
def add_soustrac_column(
    df, col_soust, col_to_soust, new_col_name=None, molecular=False
):
...
def add_field(variant_data, df, field_list, id_col="variant_id"):
    ...
    df[col_name] = df[id_col].apply(
        lambda vid: extract_field(variant_data.get(vid, {}), field_list, None)
    )

    if "exon" in col_name:
        exon_processed = df[col_name].apply(
            lambda x: process_exon_field(x) if x is not None else (None, None, None)
        )
        df[[f"{col_name}_number", f"{col_name}_total", f"{col_name}_ratio"]] = (
            pd.DataFrame(exon_processed.tolist(), index=df.index)
        )
...
def build_embedding_model(num_categories, embedding_dim):
    ...
    emb = Embedding(
        input_dim=num_categories, output_dim=embedding_dim, name="embedding_layer"
    )(input_cat)
